...
const Input = ({
    ...
    const inputPrimaryClasses = `block w-full p-3 text-sm text-tremor-content-title bg-transparent peer rounded-tremor-small outline outline-1 focus:outline-2 focus:outline-offset-[-1px] ${!errorMessage ? ' outline-tremor-outline focus:outline-tremor-brand-subtle' : ' outline-red-600 focus:outline-red-600'}`;

    // note TODO: using tailwind @apply
    const inputSecondaryClasses = `block w-full p-3 text-sm text-tremor-content-title bg-transparent peer ${!errorMessage ? ' border-0 border-b focus:border-b-2 border-gray-300 focus:outline-none focus:border-tremor-brand-subtle' : ' text-tremor-content-emphasis border-0 border-b focus:border-b-2 border-red-600 focus:outline-none focus:border-red-600'}`;
