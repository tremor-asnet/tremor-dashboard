...
const LoadingIndicator = ({
  ...
  isFullWidth ? (
    <div className="fixed bg-[rgba(0,0,0,0.5)] overflow-hidden w-full h-full inset-0 z-50 cursor-not-allowed">
      <Flex className="justify-center items-center h-full">
        <div role="status">
          <svg
            aria-hidden="true"
            className={`w-${width} h-${height} text-transparent animate-[spin_0.5s_linear_infinite] dark:text-gray-600 fill-white`}
            viewBox="0 0 22 24"
            fill="none"
    ...
    <div
      role="status"
      className={`w-${width} h-${height} text-transparent animate-[spin_0.5s_linear_infinite] dark:text-gray-600 fill-white rounded-full border-l-transparent border-r-white border-t-white border-b-transparent border-solid border-2`}>
      <span className="sr-only">Loading...</span>
    </div>
  );
