...
const AnalyticsInfo = ({ infoData }: AnalyticsInfoProps): JSX.Element => {
  const { imgSrc, handleOnError } = useImage(infoData.photo);
  const { photo, name, description, price, location, ...rest } = infoData;
    ...
    <Card className="group overflow-visible p-4 mb-12 border-none ring-0 md:w-[calc(50%-0.75rem)] md:mr-6 md:even:mr-0 md:last:mr-0 lg:even:mr-6 hover:cursor-pointer analytics-info">
      <Flex className="justify-start flex-col items-start -mt-10">
        ...
        <Flex className="flex-col pt-7 px-2">
          ...
          <Title className="w-full font-primary font-normal tracking-normal text-primary text-xl text-center leading-snug capitalize mt-8 mb-2">
            {name}
          </Title>
          <Text className="text-secondary font-primary flex-wrap text-tremor-title font-light leading-[26px] tracking-[0.17136px] text-center">
            {description}
          </Text>
        </Flex>
        ...
        <Flex className="p-2 pt-0">
          <Flex>
            <Text className="flex-wrap font-primary font-normal text-tremor-title leading-[26px] tracking-[0.17136px] text-center">
              &#36;{price}/night
            </Text>
          </Flex>
